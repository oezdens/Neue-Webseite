...
export default function LiquidEther({
	...
	useEffect(() => {
		...
		class MouseClass {
			...
			timer: any = null;
			hasUserTimer: any = null;
			container: any = null;
			_onMouseMove: any = null;
			_onTouchStart: any = null;
			_onTouchMove: any = null;
			_onMouseEnter: any = null;
			_onMouseLeave: any = null;
			_onTouchEnd: any = null;
			isHoverInside = false;
			...
			setCoords(x: number, y: number) {
				...
				if (this.hasUserTimer) clearTimeout(this.hasUserTimer);
				const rect = this.container.getBoundingClientRect();
				const nx = (x - rect.left) / rect.width;
				const ny = (y - rect.top) / rect.height;
				this.coords.set(nx * 2 - 1, -(ny * 2 - 1));
				// mark user actively controlling for a short while
				this.mouseMoved = true;
				this.hasUserControl = true;
				this.timer = setTimeout(() => {
					this.mouseMoved = false;
				}, 100);
				// after a short idle the user control flag is cleared so auto-driver can resume
				this.hasUserTimer = setTimeout(() => {
			...
			onDocumentMouseMove(event: MouseEvent) {
				...
				// If auto-driver is active, do a very short takeover (smooth but snappy)
				if (this.isAutoActive && !this.hasUserControl && !this.takeoverActive) {
					...
					// keep takeover duration short for snappy feel
					this.takeoverDuration = Math.min(this.takeoverDuration || 0.05, 0.05);
					this.hasUserControl = true;
					this.isAutoActive = false;
					return;
				}
				// Normal mouse movement: update coords directly
			...
			onTouchEnd() {
				...
				// consider touch end as end of user control
				this.hasUserControl = false;
				if (this.hasUserTimer) {
					clearTimeout(this.hasUserTimer);
					this.hasUserTimer = null;
				}
			}
			onMouseEnter() {
			...
			onMouseLeave() {
				this.isHoverInside = false;
				// leaving the area should clear immediate user control
				this.hasUserControl = false;
				if (this.hasUserTimer) {
					clearTimeout(this.hasUserTimer);
					this.hasUserTimer = null;
		...
		class AutoDriver {
			...
			update() {
				...
				// Always keep the auto-driver active so the demo runs continuously.
				if (!this.active) {
					this.active = true;
...
`;
		...
		class WebGLManager {
			...
			constructor(props: any) {
				...
				// enforce a very short takeover duration for snappy user takeover (max 50ms)
				Mouse.takeoverDuration = Math.min(props.takeoverDuration || 0.25, 0.05);
				this.lastUserInteraction = performance.now();
				Mouse.onInteract = () => {
					// record interaction time but do NOT stop the auto-driver so the
					// background keeps running continuously
					this.lastUserInteraction = performance.now();
				};
		...
		// Always run the background animation immediately (no IntersectionObserver). 
		// This ensures the fluid starts as soon as the page loads and runs continuously.
